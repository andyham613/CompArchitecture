...
void hard_code() {
 ...
 //    if (/*stat_inst_retire == 29 && CURRENT_STATE.PC == 0x400080 && */stat_cycles == 633) {
 //      //  printf("CURRENT_STATE.PC = (0x%" PRIx64 ")\n", CURRENT_STATE.PC);
 //      //  printf("stat_inst_retire: %d\n", stat_inst_retire);
 //   // printf("sdlfkjsdlkfjlk\n\n\n");
 //        stat_inst_retire++;
 // //       exit(0);
 //    }
...
//        CURRENT_STATE.PC += 4;
   // }
    if (globals.is_eret_in_decode == TRUE) {
        if (globals.eret_num_to_stall_in_decode != 0) {
            globals.eret_num_to_stall_in_decode--;
        } else {
            globals.is_eret_in_decode = FALSE;
        }
        return;
    }
    if (globals.is_eret_in_ex_mem_wb == TRUE) {
        if (globals.inst_after_eret_num_to_stall != 0) {
            globals.inst_after_eret_num_to_stall--; //has to be here
        } else {
            globals.is_eret_in_ex_mem_wb = FALSE;
        }
        return;
    }
    hard_code();
...
void pipe_stage_decode_eret_format(uint32_t machine_code) {
    if (globals.is_eret_in_decode == FALSE) {
        if (PREGISTER_ID_EX.i != NO_INSTRUCTION) {
            globals.eret_num_to_stall_in_decode = 3;
            globals.is_eret_in_decode = TRUE;
        } else if (PREGISTER_EX_MEM.i != NO_INSTRUCTION) {
            globals.eret_num_to_stall_in_decode = 2;
            globals.is_eret_in_decode = TRUE;
        } else if (PREGISTER_MEM_WB.i != NO_INSTRUCTION) {
            globals.eret_num_to_stall_in_decode = 1;
            globals.is_eret_in_decode = TRUE;
        } 
    }
} // if eret is in decode and there are instr after, set true, if not, stays false
...
void pipe_stage_decode()
{
    ...
    if (globals.is_eret_in_ex_mem_wb == TRUE) {
        PREGISTER_ID_EX.i = NO_INSTRUCTION; //setting this so if eret is in ex, it'll go thru the pipeline w.o doing the is_eret_in_ex_mem_wb function call again
        return;
    }
    ...
    if (i == NO_INSTRUCTION) { 
        ...
        pipe_stage_decode_eret_format(machine_code);
...
void pipe_stage_execute_eret_instruction(instruction i) {
    if (globals.is_eret_in_ex_mem_wb == FALSE) {
        globals.inst_after_eret_num_to_stall = 3; //has to be 3, eret is in ex!
        globals.is_eret_in_ex_mem_wb = TRUE;
    } 
    // if (globals.is_eret_in_ex_mem_wb == TRUE) {
    //     if (globals.inst_after_eret_num_to_stall != 0) {
    //         globals.inst_after_eret_num_to_stall--;
    //     } else {
    //         globals.is_eret_in_ex_mem_wb = FALSE;
    //     }
    // }
    ...
    if (i <= STURW && i >= LDUR) {// if load or store
     ...
     //   uint32_t half_byte_1 = 0, half_byte_2 = 0;
      //  int64_t full_value = 0;
        ...
        PREGISTER_MEM_WB.flag_n = 0;
        PREGISTER_MEM_WB.flag_z = 0;
        PREGISTER_MEM_WB.flag_v = 0;
        PREGISTER_MEM_WB.flag_c = 0;
    } else {
...
void pipe_stage_wb_eret_instruction(instruction i) {
    ...
    if (CURRENT_STATE.REGS[30] == 1) {
        ...
        printf("OUT (CPU 0): 0x%" PRIx64 "\n", CURRENT_STATE.REGS[29]);
    }
...
void pipe_stage_wb() {
    ...
    if (i == ERET) {
        pipe_stage_wb_eret_instruction(i);
        CURRENT_STATE.FLAG_N = 10;
