...
typedef struct
{
    ...
    /* BTB */
    int GHR;
    //int is_branch_predict_correct;
    branch_target_buffer BTB;
    pattern_history_table PHT;
} bp_t;

bp_t bpt, bpt_1, bpt_2, bpt_3; //global variables
...
void bpt_init_1();
void bp_predict_1(int64_t PC, int is_squash);
void bp_update_1(int64_t PC, instruction i, int64_t nPC_m, int is_branch_taken, int* is_correct_branch);

void bpt_init_2();
void bp_predict_2(int64_t PC, int is_squash);
void bp_update_2(int64_t PC, instruction i, int64_t nPC_m, int is_branch_taken, int* is_correct_branch);

void bpt_init_3();
void bp_predict_3(int64_t PC, int is_squash);
void bp_update_3(int64_t PC, instruction i, int64_t nPC_m, int is_branch_taken, int* is_correct_branch);
